...
void setup()
{
   ...
   while(!Serial)
   {
    ...
    while(!Serial1)
    {
      //Serial.println("not connected to xbee");
    }
...
//        }
//        Serial.println();
//        Serial.print("alreadyRead value: ");
//        Serial.println(alreadyRead);
        if(inbyte == 0x7E)
        {
            packetIndex = 1;
            string[0] = inbyte;
            ...
            Serial.println(lengthBytes[0]+lengthBytes[1]);
            int i;
            // = lengthBytes[0]+lengthBytes[1];
            for(i = 0; i < packetLength; i++)
            {
                //currentByte = readPacketByte();
                currentByte = Serial1.read();
                string[3 + i] = currentByte;
...
//        Serial.print("packet length index: ");
//        Serial.println(packetLengthIndex);
        ...
        if((packetIndex == (packetLength + 3)) && packetIndex > 2)
        {
          ...
          while(currentWord != 0xEE)
          {
            ...
            //Serial.print(" trigger Char: ");
            //Serial.print(triggerWord[countycount]);
            if( triggerWord[countycount] != (char)currentWord)
            {
              if(countycount < 7)
              {
                txTrigger = false;
            ...
            else if(triggerWord[countycount] == (char)currentWord)
            {
              if(countycount < 8)
              {
                txTrigger = true;
          ...
          //send the name of the individual transceivers
          //Transmitter address 1:
          //uint8_t address1[] = string[4-11]
