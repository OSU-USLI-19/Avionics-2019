...
switch case_t
    ...
    case 5 %% GPRMC: Recommended minimum specific GPS/Transit data
        ...
        if(isempty(t_time))
            ...
            data.BODCTime  =  datenum(t_time,'HHMMSS') - ...    %% TODO: fix milliseconds
                floor(datenum(t_time,'HHMMSS'));
        end
        clear t_time;
        ...
        %third to sixth data field are lat/lon coordinates
        ...
        % eighth data field is true course
        ...
        % nineth data field is date
        t_date  =  fields(10,1:end);
        if(~isempty(t_date))
            t_date = datenum(t_date, 'ddmmyy')
            if isnan(data.BODCTime)
                data.BODCTime  =  t_date;
            else
                data.BODCTime  =  data.BODCTime + t_date;
            end
        end
        clear t_date
        
        % tenth data field is magnetic variation degree
        t_magneticvariation_degree =  fields(11,1:end);
        if(isempty(t_magneticvariation_degree))
            data.magneticvariation.degree  =  NaN;
        else
            data.magneticvariation.degree  =  str2double(t_magneticvariation_degree);
        end
        clear t_magneticvariation_degree

        % eleventh data field is magnetic variation direction
        t_magneticvariation_direction =  strtrim(fields(12,1:end));
        if(isempty(t_magneticvariation_direction) || ~sum(strcmp(t_magneticvariation_direction, {'E', 'W'})))
            data.magneticvariation.direction  =  NaN;
        else
            data.magneticvariation.direction  =  t_magneticvariation_direction;
        end
